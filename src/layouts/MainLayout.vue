...
const vistaComponente = ref('home')
const eventos = ['mousemove', 'keydown', 'click', 'touchstart']
let inactividadTimer = null
const tiempoInactividad = ref('');
    ...
    items.value = sessionStore.menusesion.map(menu => ({
      ...
      items: menu.submenus?.map(submenu => {
        return {
          ...
          command: () => {
            vistaComponente.value = submenu.ruta
            console.log(submenu.ruta);
          }
        };
      }) || []
    }));
...
const home = async () => {
  vistaComponente.value = 'home'
}

onMounted(async () => {
  const confSis = sessionStore.conf_sis;
  const tipoInactividad = confSis.find(item => item.codigo === "tipo_ina");
  if (tipoInactividad) {
      tiempoInactividad.value= tipoInactividad.valor; // Esto será "300" en tu ejemplo
  }
  await cargamenu()
  const activa = await validarSesionActiva()
  if (!activa) {
...
<template>
    <div class="card">
        <Menubar :model="items">
            <template #start>
                <svg @click="home" width="35" height="40" viewBox="0 0 35 40" fill="none" xmlns="http://www.w3.org/2000/svg" class="h-8">
                    <path
  ...
  <Dialog v-model:visible="showSessionModal" pt:root:class="!border-0 !bg-transparent" pt:mask:class="backdrop-blur-sm">
      <template #container="{ closeCallback }">
          <div class="flex flex-col px-8 py-8 gap-6 rounded-2xl" style="background-image: radial-gradient(circle at left top, var(--p-primary-400), var(--p-primary-700))">
              ...
              <div class="flex items-center gap-4">
                  ...
                  <Button label="Sign-In"  variant="text" class="!p-4 w-full !text-primary-50 !border !border-white/30 hover:!bg-white/10"></Button>
              </div>
          </div>
      </template>
  </Dialog>
